...
return new class extends Migration
{
    ...
    public function up()
{
    Schema::create('tbl_nota_cancelacion', function (Blueprint $table) {
        ...
        $table->foreignId('user_id') // Agrega una columna para la relación con la tabla users
              ...
              ->onDelete('cascade'); // Si el usuario se elimina, también se elimina la nota de cancelación
        $table->foreignId('id_solicitud') // Agrega la clave foránea hacia tbl_solicitudes
              ->constrained('tbl_solicitudes', 'id_solicitud') // Relaciona con la tabla tbl_solicitudes y usa 'id_solicitud' como columna
              ->onDelete('cascade'); // Si la solicitud se elimina, también se eliminará la nota de cancelación
    ...
    {
        Schema::dropIfExists('tbl_nota_cancelacion');
    }
};
